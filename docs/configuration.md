...
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, and `cmux` for ordinary ship and scout tasks; verifier scouts launched in neutral non-repository directories accept every one except Orca, while `backend=orca` and `backend=cmux` both still refuse `--secondmate` until secondmate launch semantics are designed for each.
...
FM_BACKEND=             # optional runtime backend override for new spawns; tmux/herdr/zellij/orca/cmux support ordinary ship/scout spawns, neutral verifier scouts exclude orca, codex-app is not accepted
...
FM_VERIFY_SPAWN_OVERRIDE=      # test seam: replaces bin/fm-spawn.sh as the verifier launcher
FM_VERIFY_PS_OVERRIDE=         # test seam: replaces /bin/ps in the verifier build runner's descendant scan
FM_VERIFY_LSOF_OVERRIDE=       # test seam: replaces lsof in the verifier build runner's descendant scan
...
FM_WORKING_STALE_ESCALATE_SECS=1500  # idle seconds before a provably-working stale pane's FIRST wedge escalation; repeats for the same unbroken condition double the wait, capped at FM_BUSY_TURN_MAX_SECS; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
FM_STALE_ESCALATE_SECS=240         # watcher pause-recheck throttle and the away-mode daemon's idle seconds before a stale pane escalates as a possible wedge; when set explicitly it also binds the provably-working watcher pace above for compatibility
