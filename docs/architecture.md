...
Actionable wakes include captain-relevant status signals, no-verb signals carrying any task that is neither provably working nor declared paused, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_WORKING_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Repeated provably-working stale escalations on the same unchanged pane back off (the wait doubles per repeat, capped at `FM_BUSY_TURN_MAX_SECS`, because the crew is provably working and a repeat wake adds no new information), add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
The cap bounds that backoff growth only, and each escalation restarts the wedge timer, so at the shipped defaults the escalation count reaches `demand-deep-inspection` after 1500+3000+3600 = 8100s of unbroken condition.
...
That bound starts the wedge timer rather than surfacing directly, so at the shipped defaults such a pane first surfaces at 3600+1500 = 5100s.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when every task the wake references falls into one of two absorb classes: `bin/fm-crew-state.sh` reports positive evidence that the crew is still working (an actively running no-mistakes step attributed to that crew's current code, or an exact busy verdict from the semantic busy-state contract), or that task's current last status line is a declared `paused:`.
...
Those classes are decided per task rather than over the whole coalesced batch, so a paused crew's append that lands inside the same `FM_SIGNAL_GRACE` window as another crew's turn-end is absorbed with it; the batch surfaces when any single task found no absorb class of its own, which is also how a crew that finishes always surfaces regardless of how absorbable its companions are.
The bounded `FM_PAUSE_RESURFACE_SECS` recheck, measured from when the pause was first observed, is the only look a task absorbed as a declared pause is guaranteed, and only those tasks join that cadence.
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint; a confidently dead agent is recovered straight onto it with no look of its own, because an idle endpoint exposes no decision gate to surface.
A declared wait that authoritative state does not confirm - a still-live agent that may be parked at an interactive decision gate the pause line does not describe, a momentarily unreadable crew state, and always a `captain-held:` line, since `bin/fm-crew-state.sh` has no case for that verb - takes exactly one fail-open look on the pause episode's first sighting and then joins the same bounded cadence, so ordinary pane repainting no longer re-opens that look the way one look per distinct pane hash did; an active run-step or busy pane still outranks the log line, and the secondmate idle-endpoint exemption is unchanged.
Both supervisors classify a crew by its current last status line only, so a crew that appends `resolved:` while still waiting on the same external thing reads as neither paused nor working and surfaces.
The crew-side status protocol owns that ordering rule - `bin/fm-brief.sh`'s generated scaffolds require the last line to describe the state the crew is actually in when its turn ends - rather than the classifier looking back past a resolution, which would resurrect the answered decision as the current state.
Away mode is the one supervision path that does not honour any of these absorb classes: while `state/.afk` exists the watcher is one-shot, queues every wake, and its daemon owns triage.
Every other arm path - the shared arm wrapper `bin/fm-watch-arm.sh`, the Claude Stop auto-arm, and the bounded foreground checkpoint `bin/fm-watch-checkpoint.sh` that Codex arms through - runs the same `bin/fm-watch.sh` triage and therefore honours them unchanged.
...
The header of `bin/fm-tmux-lib.sh` owns the tmux submit core's harness-gated busy-queued Enter exception; the separate OpenCode gap on Herdr remains in `docs/herdr-backend.md`.
...
Independently, `fm-spawn.sh`, `fm-send.sh`, `fm-verify.sh`, and `fm-teardown.sh` source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
