...
A declared wait that authoritative state does not confirm - typically a still-live agent that may be parked at an interactive decision gate the pause line does not describe - stays fail-open, but that fail-open look is now taken once per pause episode rather than once per distinct pane hash, so ordinary pane repainting no longer re-opens it; the secondmate idle-endpoint exemption is unchanged.
